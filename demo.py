    ...
    def __init__(self,
        ...
        for i in range(JobNum):
            for j in  range(DeviceNum):
                f = FCLS(
                    path=[self.links[2*j],self.boxes[0] ,self.links[-2], ], 
                    rpath=[self.links[-1],self.boxes[0] ,self.links[2*j+1], ],
                    nic_rate_bps=1.2*bw_bps, #2*
                    job_id=i,
                    #start_time=0, 
                    #expired_time=20,
        ...
        M_and_BDP = max_mup_cache_size + (bw_bps*ideal_rtt*JobNum)/INA_PAATP.PING_PKT_SIZE_IN_BITS
