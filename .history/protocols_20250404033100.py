...
class Muilt(INA_PAATP):
    ...
    def send_ping(self, chunk_seq,ping_seq,quantify_type,resend_flag, delay=0.0):
        cur_time = self.get_cur_time()
        self.send_times[ping_seq] = cur_time
        if chunk_seq not in self.chunk_send_times:
            self.chunk_send_times[chunk_seq] = cur_time
        ...
        if(pkt.quantity_type == self.type_a or pkt.quantity_type == self.type_ab or 
           pkt.quantity_type == self.type_abc or pkt.quantity_type == self.type_abcd): 
            self.ping_yet_unpong[pkt.chunk_seq] = pkt
            self.out_of_order_cnts[pkt.chunk_seq] = 0
        ...
        obj = pkt.get_next_hop()
        print(f"flow {self.id} send ping {pkt.ping_seq} chunk_seq {pkt.chunk_seq} type {pkt.quantify_type}")
        #delay = random.random() * 0.05 * self.get_sending_interval()
        e = Event(cur_time + delay, obj, 'on_pkt_received', params=dict(pkt=pkt))
        #print('# sent ping', pkt.ping_seq, 'chunk_seq', chunk_seq, 'cwnd', self.cwnd, 'received_pong', self.received_pong)
        return e, pkt
                ...
                if flow_id not in self.mup_cache_meta.get(chunk_key, []):
                    ...
                    if not self.mup_cache.has_cache(job_id, pkt.chunk_seq,flow_id):
                        ...
                        if(flow_for_id == None):
                            ...
                            awd_pkt = Packet(sent_time=flow_for_id.send_times[pkt.chunk_seq],
