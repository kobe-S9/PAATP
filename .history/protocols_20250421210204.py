...
class Muilt(PAATP):
    ...
    def send_ping(self, chunk_seq,ping_seq,quantify_type,resend_flag, delay=0.0):
        ...
        self.cwd_out_of_order_cnts[pkt.ping_seq] = 0
    ...
    def on_cwd_received(self, pkt):
        ...
        if pkt.ping_seq in self.ping_yet_uncwd:
            ...
            del self.cwd_out_of_order_cnts[pkt.ping_seq]
        elif pkt.ping_seq in self.resend_queue:
            del self.resend_queue[pkt.ping_seq]
        # self.check_completed()
        if self.state != self.ONLINE:
            return []
    ...
    def on_pong_received(self, pkt):
        ...
        to_resend = []
        for i, ipkt in self.ping_yet_unpong.items():
            if ipkt.chunk_seq < pkt.chunk_seq:
                self.out_of_order_cnts[ipkt.chunk_seq ] += 1
                if self.out_of_order_cnts[ipkt.chunk_seq ] >= 1:
                    to_resend.append(ipkt.chunk_seq )
                    del self.out_of_order_cnts[ipkt.chunk_seq ]

        for chunk_seq in to_resend:
            resend_ping_seqs = []
            for ping_seq, (chunkseq, quantify_type,agg_comp_flag) in self.ping_seq_to_chunk_seq.items():
                if chunkseq == chunk_seq and agg_comp_flag == 0:
                    resend_ping_seqs.append(ping_seq)
            if len(resend_ping_seqs) == 0:
                if (-chunk_seq-1) not in self.resend_queue:
                    to_resend.append(-chunk_seq-1)#用来表示ack包丢了而不是cwd包丢了-1是因为chunkseq从0开始
                    self.resend_queue[-chunk_seq-1] = 1
                    if chunk_seq in self.ping_yet_unpong:
                        del self.ping_yet_unpong[chunk_seq]
                    if -chunk_seq-1 in self.ping_yet_uncwd:
                        del self.ping_yet_uncwd[-chunk_seq-1]
            else:
