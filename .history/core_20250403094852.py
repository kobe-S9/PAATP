...
class SPQ(Qdisc):
    ...
    def enq(self, pkt):
        ...
        if p >= random.random():
            ...
            return
        """
        if self.mq[i].total_pkt_size_in_bits > self.mq_sizes[i]:
            pkt.drop(pkt.DROP_CAUSED_BY_CONGESTION)
            return
        """
        self.mq[i].enq(pkt)
...
class Link(object):
    ...
    def pkt_enq(self, pkt):
        ...
        if random.random() < self.lr:
            ...
            print('# link', self.id, 'received packet', pkt.chunk_seq, "flow_id",pkt.flow.id)
            self.qdisc.enq(pkt)
...
class Middlebox(object):
    ...
    def on_pkt_received(self, pkt):
        print('# Middlebox received packet', pkt.chunk_seq, "flow_id",pkt.flow.id)
...
class Flow(object):
    ...
    def on_pkt_lost(self, reason, pkt=None):
        ...
        print('# drop packet for flow', self.id, pkt.chunk_seq, self.lost_reason)
        #raise ValueError
